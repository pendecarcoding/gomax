...
<div class="aiz-titlebar text-left mt-2 mb-3">
    <div class="row align-items-center">
        <div class="col-auto">
            <h1 class="h3">{{translate('All Patners')}}</h1>
        </div>
        @can('add_patners')
            <div class="col text-right">
                <a href="{{ route('patners.create') }}" class="btn btn-circle btn-info">
                    <span>{{translate('Add New Patners')}}</span>
                </a>
            </div>
        @endcan
    </div>
    ...
    <form class="" id="sort_patners" action="" method="GET">
        <div class="card-header row gutters-5">
            <div class="col text-center text-md-left">
                <h5 class="mb-md-0 h6">{{ translate('All Patners ') }}</h5>
            </div>
            ...
            <table class="table mb-0 aiz-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            @can('edit_patners')
                                <a class="btn btn-soft-primary btn-icon btn-circle btn-sm"  href="{{ route('patners.edit',base64_encode($v->id))}}" title="{{ translate('Edit') }}">
                                    <i class="las la-pen"></i>
                                </a>
                            @endcan
                            @can('delete_patners')
                                <a href="#" class="btn btn-soft-danger btn-icon btn-circle btn-sm confirm-delete" data-href="{{route('patners.destroy', base64_encode($v->id))}}" title="{{ translate('Delete') }}">
